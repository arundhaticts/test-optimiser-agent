"""Synthetic sample-data generator — the single source of truth for the demo fixture.
...
Architecture position: sample_data/ = the synthetic fixture. This script is the
single-source-of-truth generator, run by a DEVELOPER, not part of an agent run. It emits
everything under sample_data/ EXCEPT this file itself. NOTE: the many string/f-string
literals below are the SOURCE-CODE TEMPLATES written into the suite and JSON — they are
data, not the generator's own logic; only the generator's Python code is documented here.

Called by: a developer, manually (`python sample_data/generate_sample_data.py`). The agent
never imports or runs this; the tests never call it (they read its outputs).

Data in: the planted-fact constants declared below (DUPLICATE_PAIRS, FLAKY, SLOW, CRITERIA,
GAP_IDS, FILES).
Data out: writes sample_suite/test_*.py (the suite), mock_ci_history.json,
sample_criteria.json, expected_findings.json (the golden set), and README.md — all derived
from the same constants so they stay in sync.
...
def _all_tests():
    """Yield every test tuple across all files in declaration order.

    Purpose: flatten FILES (filename -> (desc, [tests])) into a single ordered stream so
        CI history and README builders can iterate tests without caring which file they
        live in.
    Inputs: none (reads the module-level FILES mapping).
    Outputs: a generator of (name, docstring, body_lines) tuples, order preserved.
    Side effects: None (pure).
    Called by: write_ci_history.
    Calls: dict.items.
    """
    # WHY: iterate files then tests so ordering is stable and deterministic (matches the
    # order the suite files are written, keeping generated artifacts reproducible).
...
def write_suite() -> None:
    """Write the pytest suite under sample_suite/ from the FILES constant.

    Purpose: turn the declared FILES mapping into runnable pytest files — the suite the
        agent's intake node parses. Each test becomes a `def test_*()` with its docstring
        and body lines. The planted near-duplicates live here (partners share a docstring).
    Inputs: none (reads FILES, SUITE_DIR).
    Outputs: None (return); writes sample_suite/test_*.py.
    Side effects: file writes — creates SUITE_DIR, deletes stale test_*.py, writes one file
        per entry in FILES.
    Called by: main.
    Calls: Path.mkdir/glob/unlink/write_text, str.join.
    """
    SUITE_DIR.mkdir(parents=True, exist_ok=True)
    # WHY: clear stale generated files first so a renamed/removed test can't linger and
    # collide with the new set (test ids must be unique across the whole suite).
    for old in SUITE_DIR.glob("test_*.py"):
        old.unlink()

    # WHY: one output file per FILES entry; build its lines, then write. The header + body
    # strings below are the emitted SOURCE TEMPLATE (data), not this function's logic.
    for fname, (desc, tests) in FILES.items():
        ...
        # WHY: emit each test as a def + docstring + indented body — the docstring is what
        # the agent matches on for coverage/duplicate detection, so it is always present.
...
def write_ci_history() -> None:
    """Write mock_ci_history.json — the flaky/slow signal for the redundancy node.

    Purpose: emit a per-test {runs, fails, avg_seconds} record so redundancy detection has
        CI stats without a real CI. Planted flaky tests get a high fail count; planted slow
        tests get a high avg time; every other test gets a stable, fast, seeded record.
    Inputs: none (reads FLAKY/SLOW maps, SEED, RUNS, all tests via _all_tests).
    Outputs: None (return); writes mock_ci_history.json.
    Side effects: file write.
    Called by: main.
    Calls: random.Random, _all_tests, json.dumps, Path.write_text.
    """
    # WHY: seed the RNG so the "boring filler" (stable tests' sub-second times / 0-1 fails)
    # is byte-for-byte reproducible; only planted facts are exact, the rest is deterministic.
    ...
    # WHY: give every test exactly one CI record. Planted flaky -> high fails (fast so it
    # trips ONLY the flaky detector); planted slow -> 0 fails + high avg_seconds (trips ONLY
    # the slow detector); everything else -> stable + fast so it is flagged by neither.
...
def write_criteria() -> None:
    """Write sample_criteria.json — the acceptance criteria for the coverage node.

    Purpose: emit the AC-* criteria the suite is matched against. AC-1..AC-5 map to existing
        tests; the GAP_IDS criteria are the planted coverage gaps (no test covers them).
    Inputs: none (reads CRITERIA, GAP_IDS).
    Outputs: None (return); writes sample_criteria.json.
    Side effects: file write.
    Called by: main.
    Calls: json.dumps, Path.write_text.
    """
    # WHY: the planted gaps exist so the coverage node has a known-uncovered requirement to
    # find; they are worded so no test in the suite matches them above the gap threshold.
...
def write_golden() -> None:
    """Write expected_findings.json — the golden answer key (test-only, never an input).

    Golden answer key — derived from the SAME constants, so it cannot drift.

    Purpose: record the findings we planted (duplicates/flaky/slow/coverage_gaps) in a
        compact shape so tests/test_graph_e2e.py can grade the agent's output against them.
    Inputs: none (reads DUPLICATE_PAIRS, FLAKY, SLOW, CRITERIA, GAP_IDS, RUNS).
    Outputs: None (return); writes expected_findings.json.
    Side effects: file write.
    Called by: main.
    Calls: round, json.dumps, Path.write_text.
    """
    # WHY: build the answer key from the same planted constants that drive the data files —
    # this is what keeps the golden set and the generated fixtures perfectly in sync.
    golden = {
...
def write_readme() -> None:
    """Write README.md — the human explainer for the sample_data/ fixture.

    Purpose: render a detailed, always-up-to-date README describing how the data is
        generated, each file, and what the app/tests read — built from the same constants
        so the prose never disagrees with the data.
    Inputs: none (reads FILES, DUPLICATE_PAIRS, FLAKY, SLOW, CRITERIA, GAP_IDS, RISK_HINT,
        RUNS).
    Outputs: None (return); writes README.md.
    Side effects: file write.
    Called by: main.
    Calls: sum, dict, str.join, Path.write_text.
    NOTE: the `L` list below is the README's Markdown CONTENT (data templates), not this
        function's logic — comments here only explain the Python that assembles it.
    """
    # WHY: precompute lookups the README lines interpolate — the total test count, each
    # duplicate's partner (both directions), and flaky/slow maps — so the prose is derived
    # from the same planted constants and can't drift from the generated files.
...
def main() -> None:
    """Regenerate the entire sample_data/ fixture, then print a summary.

    Purpose: developer entrypoint — write all five outputs (suite, CI history, criteria,
        golden set, README) from the planted constants, keeping them in sync, then echo a
        summary of what was planted.
    Inputs: none (reads the module-level constants).
    Outputs: None; writes the five artifacts and prints a summary to stdout.
    Side effects: file writes (via the write_* helpers); stdout prints.
    Called by: `__main__` guard (a developer running the script).
    Calls: write_suite, write_ci_history, write_criteria, write_golden, write_readme, print.
    """
    # WHY: order is deliberate but independent — the suite first, then the derived JSON
    # signals, then the golden key and README that summarise everything.
    write_suite()
    write_ci_history()
    write_criteria()
    write_golden()
    ...
    # WHY: print a plain-text summary so a developer can eyeball what was regenerated and
    # confirm the planted facts match their intent before committing the fixture.
    total = sum(len(tests) for _d, tests in FILES.values())
